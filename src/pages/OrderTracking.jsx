...
function OrderTracking() {
  ...
  const lastOrder = React.useMemo(() => {
    try {
      return JSON.parse(localStorage.getItem('xyloLastOrder')) || {};
    } catch (error) {
      return {};
    }
  }, []);
  ...
  const fetchTracking = React.useCallback(
    async (payload, { silent = false } = {}) => {
      ...
      try {
        ...
        try {
          localStorage.setItem('xyloLastOrder', JSON.stringify({ orderNumber: data.orderNumber, email: data.email }));
        } catch (storageError) {
  ...
  return (
    <Container maxWidth="md" sx={{ py: { xs: 6, md: 10 } }}>
      <Stack spacing={3} alignItems="center" textAlign="center" sx={{ mb: 4 }}>
        <Chip label={t('orderTracking')} color="primary" variant="outlined" />
              ...
              <Stack spacing={2}>
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                  <Button type="submit" variant="contained" size="large" disabled={loading}>
                    {loading ? t('updating') : t('trackMyOrder')}
                  </Button>
                  {trackingData && (
          ...
          <Grid item xs={12} md={6}>
            <Stack spacing={2}>
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="caption" color="text.secondary">
                  thameejahamed73@gmail.com • +1 (833) 555-0195
                </Typography>
              </Stack>
              {trackingData?.estimatedDelivery && (
                <Typography variant="caption" color="text.secondary">
                  {t('estimatedDelivery')}: {formatTimestamp(trackingData.estimatedDelivery)}
                </Typography>
              )}
              {errorMessage && (
      ...
      <Paper elevation={0} sx={{ mt: 5, p: { xs: 3, md: 5 }, borderRadius: 4, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
          <Typography variant="h6" fontWeight={700}>
            {hasTracking ? t('liveJourney') : t('whatToExpect')}
          </Typography>
